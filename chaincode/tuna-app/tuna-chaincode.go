...
func (s *SmartContract) queryHash(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if hashAsBytes == nil {
		return shim.Error("Could not locate hash")
	}
 ...
 */
func (s *SmartContract) recordHash(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments. Expecting 2")
	}

	var hash = Hashrecord{Hash: args[1] }

	hashrecordAsBytes, _ := json.Marshal(hash)
	err := APIstub.PutState(args[0],hashrecordAsBytes )
	if err != nil {
